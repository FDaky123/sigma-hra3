...
// ========== VLNA INFO ==========
if (instance_exists(Owave_manager) && room != Room_boss && room != Room_shop) {
    draw_set_halign(fa_left);
    draw_set_valign(fa_top);
    draw_set_color(c_white);
    ...
    // Enemy info během wave
    if (Owave_manager.wave_active) {
        // SPOČÍTEJ ACTUAL COUNT
        var actual_count = instance_number(Oenemy);
        
        // UKAŽ ACTUAL COUNT (ne tracked enemies_alive)
        draw_text(20, 90, "Enemies: " + string(actual_count) + " / " + string(Owave_manager.enemies_to_spawn));
    }
    
    // Wave complete
    if (Owave_manager.wave_complete && Owave_manager.current_wave < 4) {
        draw_set_color(c_lime);
    ...
    if (Owave_manager.wave_complete && Owave_manager.current_wave == 4) {
        draw_set_color(c_red);
        draw_text(20, 115, "ALL WAVES COMPLETE!");
        draw_set_color(c_yellow);
        draw_text(20, 135, "Go to shop - BOSS AWAITS!");
    }
    ...
    if (Owave_manager.auto_start_timer > 0 && !Owave_manager.wave_active) {
        ...
        draw_set_valign(fa_top);
        draw_set_color(c_yellow);
        var seconds = ceil(Owave_manager.auto_start_timer / 60);
